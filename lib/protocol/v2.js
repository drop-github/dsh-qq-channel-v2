...
import { TIMEOUT_PROMPT_MS, TIMEOUT_COMMAND_MS } from './transport.js';
...
export function createV2({ transport, mux, log }) {
  return {
    ...
    /**
     * 命令通道（v2.0.3）：把 `/compact`、`/goal` 这类**宿主直接执行、不进模型**的命令交给
     * dsh-commands 的注册表。args 键名来自真机核证：`{ agentId, line, submittedAttachments }`
     * （agentId 就是会话 id）。宿主对"语法不认识 / 名字没注册"**静默返回空值**（连 value 都没有），
     * 调用方据此退回 prompt —— 判据在 handlers/command.js。
     */
    command: ({ agentId, line }) =>
      transport.call('commands/execute', { agentId, line, submittedAttachments: [] }, { timeoutMs: TIMEOUT_COMMAND_MS }),
