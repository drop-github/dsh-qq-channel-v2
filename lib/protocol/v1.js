...
export function createV1({ transport, auth, dshUrl, log }) {
  ...
  return {
    ...
    /**
     * v1 宿主（DSH ≤0.1.1）没有命令注册表：按"宿主没有这条通道"回报，
     * 上层于是把 `/compact` 当普通文本发出去（与升级前一致），不吞消息、不报假错。
     */
    command: () => Promise.resolve(fail('not-found', 'protocol-unsupported', 'v1 host has no command channel')),
