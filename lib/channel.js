...
export function createChannel({ ctx, config, log }) {
  ...
  const dsh = {
    ...
    createSession: () => trackRpc('session/create', () => client.create({})),
  ...
  /** v1 帧 → 统一模型 / 控制路径（字段依据 v1.2.4 反推，PROTOCOL.md §10.3）。 */
  function onV1Frame(frame) {
    const payload = frame?.payload;
    if (!payload) return;
    if (payload.type === 'session/event') {
      if (!isManaged(payload.sessionId)) return;
      pump.push(payload.sessionId, payload.event, 'live', 'v1');
      return;
    }
    if (payload.type === 'approval/requested') {
      dshHandler.onControlFrame({
        type: 'waterfall',
  ...
  async function boot() {
    // 先恢复待补发队列，再连通任何东西：崩溃/卡死留下的消息必须在日志里立刻可见。
    const recovered = pendingInbox.restore();
    if (recovered.size > 0) {
    ...
    timers.sweep = setInterval(() => {
      ...
      if (expired > 0 || evicted > 0) log?.info?.('sweep done', { expired, evicted });
    }, SWEEP_INTERVAL_MS);
    log?.info?.('channel up', {
