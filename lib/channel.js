...
/** 宿主服务晚就绪时的自举重试：间隔与次数上限（~15s，足够覆盖"约 1s 才就绪"的真机时序）。 */
export const BOOT_RETRY_MS = 1000;
export const BOOT_RETRY_MAX = 15;
...
export function createChannel({ ctx, config, log }) {
  ...
  let bootAttempts = 0;
  const timers = { reconnect: null, sweep: null, boot: null };
  ...
  const dsh = {
    ...
    // 命令通道（v2.0.3）：`/compact`、`/goal` 这些由宿主直接执行，不进模型。
    command: ({ agentId, line }) => trackRpc('commands/execute', () => client.command({ agentId, line })),
    qqAuth: () => token.ensure(),
  ...
  /**
   * 连通宿主（auth → 协议探测 → 客户端/事件泵/网关）。
   *
   * 宿主服务（webServer / connection）在 `apply()` 之后约 1s 才出现（真机 6 次启动一致）。
   * DSH ≤0.1.6 时这次抢跑会被"settings 注册成功 → 生效配置变了 → 重建通道"顺手救回来；
   * 但 **DSH ≥0.1.7 的配置由宿主托管、不再触发重建**，重试必须自己扛 ——
   * 否则升级后一旦抢跑，QQ 通道会一直哑到下次重启（真 0.1.7-alpha.1 预飞已复现）。
   */
  ...
  /**
   * 宿主服务晚就绪时的有界自举重试（见 `connect` 的说明）。
   * 锁只抢一次：`boot()` 已经抢过，重试只重连、不再碰锁。
   */
  function scheduleBootRetry(reason) {
    if (disposed || timers.boot) return;
    if (bootAttempts >= BOOT_RETRY_MAX) {
      log?.error?.('boot retries exhausted — channel stays down until a restart', { attempts: bootAttempts, reason });
      return;
    }
    bootAttempts += 1;
    log?.warn?.('host services not ready — retrying channel boot', {
      attempt: bootAttempts,
      max: BOOT_RETRY_MAX,
      inMs: BOOT_RETRY_MS,
      reason,
    });
    timers.boot = setTimeout(() => {
      timers.boot = null;
      if (disposed) return;
      connect('retry').catch((err) => log?.error?.('channel boot retry failed', { error: String(err?.stack ?? err) }));
    }, BOOT_RETRY_MS);
    timers.boot.unref?.();
  }
