...
export function createSessionStore({
   ...
   * `durableIds`：这条消息在待补发队列（inbox-store）里的落盘 id —— 忙时并入等待**不等于送达**，
   * 记录必须留着；id 随队一起走，等合并轮真正送达时再清（T-W10 泄漏回归）。
   */
  function noteInbound(sessionId, { target, parts, durableIds = [] }, now = Date.now()) {
    const entry = record(sessionId);
    ...
    entry.inboundQueue.push({
      target,
      parts,
      at: Date.now(),
      durableIds: Array.isArray(durableIds) ? durableIds : [],
    });
    let dropped = 0;
