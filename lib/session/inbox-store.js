...
export function createDurableInbox({
   ...
   * `dropWhere(item) => 原因|null`：返回原因则丢弃该条并落一条 `remove`。
   * 调用方用它清掉"按设计永远补发不到"的记录（来源→会话身份重算过的老条目）。
   */
  function restore(now = Date.now(), { dropWhere = null } = {}) {
    let fromSnapshot = 0;
    ...
    // 先清"补发不到的僵尸"，再判断"重启后还剩几条"——否则每次启动都会为它们误报一次。
    let dropped = 0;
    if (typeof dropWhere === 'function') {
      for (const [msgId, item] of [...items]) {
        let reason = null;
        try { reason = dropWhere(item); } catch { reason = null; }
        if (!reason) continue;
        items.delete(msgId);
        appendOp({ op: 'remove', msgId });
        dropped += 1;
        log?.warn?.('pending inbound dropped — unreachable by design', {
          msgId, session: item.sessionId, source: item.sourceKey, reason,
        });
      }
    }
    ...
    return { size: items.size, fromSnapshot, fromOps, pruned, dropped };
  }
