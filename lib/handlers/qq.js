...
export function createQqHandler({ log, config, state, pending, port, dsh, fetcher, pendingInbox = null, questionFlow }) {
  ...
  // 命令通道（`/compact` 等由宿主直接执行的命令）在 handlers/command.js。
  const runCommand = createCommandRunner({ log, dsh });
  ...
  async function onMessage(descriptor) {
    ...
    // 命令通道：`/compact`、`/goal`、`/feedback` 这类命令由**宿主直接执行、不进模型**。
    // 判据（小写命令名 + 空白/行尾）与宿主逐字一致；宿主不认识的行在 runner 里返回 null → 照常走 prompt。
    // 带附件的消息不走这里：命令通道没有附件接收面，别把用户发的图片悄悄吞掉。
    if (attachments.length === 0) {
      const command = await runCommand(sessionId, text);
      if (command) {
        await port.sendText(replyTarget, command.reply, { passive: { msgId } });
        return;
      }
    }
