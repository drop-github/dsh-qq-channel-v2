...
import { approvalKeyboard } from '../qq/keyboard.js';
...
export function createDshHandler({ log, config, state, pending, port, dsh, isManaged, onTurnEnd, questionFlow }) {
  ...
  async function onQuestionRequest(value) {
    ...
    // 多问题在这里被拆成"一个个问"；选项与自由输入都由 question-flow 统一收口。
    await questionFlow.start(added.value.entry);
  }
